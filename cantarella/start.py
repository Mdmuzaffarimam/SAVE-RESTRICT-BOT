...
<b>📸 QR Code:</b> <a href="{qr_code}">Scan to Pay</a>
...
async def handle_restricted_content(client: Client, acc, message: Message, chat_target, msgid):
    ...
    try:
        ...
        if custom_caption:
            caption_style = await db.get_caption_style(message.from_user.id)
            variables = extract_metadata(
                msg,
                file_name=file.split("/")[-1],
                file_size=file_size,
                duration=getattr(msg.video, "duration", 0) if msg_type == "Video" else 0,
                height=getattr(msg.video, "height", 0) if msg_type == "Video" else 0,
                width=getattr(msg.video, "width", 0) if msg_type == "Video" else 0,
            )
            # Keep legacy {filename}/{size} placeholders working too.
            variables["filename"] = variables["file_name"]
            variables["size"] = variables["file_size"]
            final_caption = render_caption(custom_caption, variables, caption_style)
        else:
        ...
        # Forward a copy to the user's configured dump chat, if any.
        # (Previously /setchat only stored the value — it was never read
        # here, so dump chat forwarding had no effect at all.)
        if sent_msg:
            try:
